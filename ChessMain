"""
This is our main driver file. It will responsible for handing user input and displaying the current GameState object.
"""
...
def main():
    ...
    loadImages() #only do this onec, before the while loop
    running = True
...
def highlightSquares(screen, gs, validMoves, sqSelected):
    if sqSelected != ():
        r, c = sqSelected
        if gs.board[r][c] == ('w' if gs.whiteToMove else 'b'): #sqSelected in a piece that can be moved
...
def drawMoveLog(screen, gs, font):
    ...
    for i in range(0, len(moveLog), 2):
        moveString = str(i // 2) + ". " + moveLog[i].getChessNotation() + " "
...
def animateMove(move, screen, board, clock):
    ...
    for frame in range(frameCount + 1):
        r, c = (move.startRow + dR * frame/frameCount, move.startCol + dC * frameCount )
        drawBoard(screen)
        ...
        if move.pieceCaptured != '--':
            ...
            screen.bilt(IMAGES[move.pieceCaptured], endSquare)
        #draw moving piece
